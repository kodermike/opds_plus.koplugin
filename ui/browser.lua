...
function OPDSBrowser:editCatalogFromInput(fields, item, no_refresh)
  ...
  local new_idx, itemnumber, should_refresh =
      CatalogManager.editCatalogFromInput(self.servers, self.item_table, fields, item, no_refresh)
...
function OPDSBrowser:fetchFeed(item_url, headers_only)
  return FeedFetcher.fetchFeed(item_url, headers_only, self.root_catalog_username, self.root_catalog_password)
end

function OPDSBrowser:parseFeed(item_url)
  return FeedFetcher.parseFeed(item_url, self.root_catalog_username, self.root_catalog_password, function(...)
    self:_debugLog(...)
  end)
end

function OPDSBrowser:getServerFileName(item_url, filetype)
  return FeedFetcher.getServerFileName(item_url, filetype, self.root_catalog_username, self.root_catalog_password)
end

function OPDSBrowser:getSearchTemplate(osd_url)
  return FeedFetcher.getSearchTemplate(
    osd_url,
    self.search_template_type,
...
function OPDSBrowser:genItemTableFromURL(item_url)
  return FeedFetcher.genItemTableFromURL(
    item_url,
    self.root_catalog_username,
    self.root_catalog_password,
    function(...)
...
function OPDSBrowser:showDownloads(item)
  ...
  local function createTitle(path, file)
    return T(
      _("Download folder:\n%1\n\nDownload filename:\n%2\n\nDownload file type:"),
      BD.dirpath(path),
      file or _(Constants.DEFAULT_FILENAME)
    )
  end
...
function OPDSBrowser:onMenuHold(item)
  if #self.paths > 0 or item.idx == 1 then
    return true
  end -- not root list or Downloads item
